...
## Real-World Context: Why PMT Chose `IDENTITY` over `SEQUENCE`

The `pmt` project (Spring Boot + PostgreSQL) uses `GenerationType.IDENTITY` for all entities via a shared `BaseEntity`. This section explains exactly why that choice was made over `SEQUENCE`, and how Hibernate's annotation maps to what PostgreSQL actually executes.

### The DB Side: `GENERATED BY DEFAULT AS IDENTITY`

All 8 tables define their primary key as a PostgreSQL **identity column**:
...
ALTER TABLE "TICKET"           ALTER COLUMN ID ADD GENERATED BY DEFAULT AS IDENTITY;
ALTER TABLE "TICKET_ACTIVITY"  ALTER COLUMN ID ADD GENERATED BY DEFAULT AS IDENTITY;
-- ... same for CHAT_ROOM, CHAT_MESSAGE, AI_LOG, PROJECT_MEMBER
```

`GENERATED BY DEFAULT AS IDENTITY` is the **SQL Standard (SQL:2003)** equivalent of `SERIAL`. Under the hood PostgreSQL still creates a hidden sequence object — but the syntax is portable and more explicit.

| Variant | Behavior |
| :--- | :--- |
| `GENERATED ALWAYS AS IDENTITY` | DB always assigns the value. An explicit `INSERT INTO t (id) VALUES (5)` fails with an error. |
| `GENERATED BY DEFAULT AS IDENTITY` | DB generates by default, but you can override explicitly — useful when seeding data or resetting sequences after a migration. |

### The Hibernate Side: `BaseEntity`

All entities in `pmt` extend a single `@MappedSuperclass` that wires up the annotation:
...
public abstract class BaseEntity {
    ...
    @Id
    @GeneratedValue(strategy = GenerationType.IDENTITY)  // DB assigns this after INSERT
    @Column(name = "ID", columnDefinition = "INTEGER")
    private Integer id;  // INT (4 bytes) — sufficient for a PM tool
...
When `GenerationType.IDENTITY` is set, the full sequence of events for a single `save()` is:

```
ticketRepository.save(newTicket)
  │
  ├─ Hibernate: ID is not set → strategy = IDENTITY → must INSERT immediately (no pre-fetch)
  │
  ├─ SQL sent:
  │   INSERT INTO "TICKET" (PROJECT_ID, TITLE, STATUS, ...) VALUES (1, 'Fix login bug', 'OPEN', ...)
  │   (notice: no ID column in the statement)
  │
  ├─ PostgreSQL: identity sequence fires → assigns id = 43 automatically
  │
  └─ Hibernate: calls Statement#getGeneratedKeys() → reads 43 → sets ticket.id = 43
```

Compare with what `SEQUENCE` would do instead:

```
ticketRepository.save(newTicket)
  │
  ├─ Hibernate: ID is not set → strategy = SEQUENCE → call sequence FIRST
  │
  ├─ SQL: SELECT nextval('ticket_id_seq')  →  returns 43
  │
  ├─ Hibernate: sets ticket.id = 43 BEFORE the INSERT
  │
  └─ SQL: INSERT INTO "TICKET" (ID, PROJECT_ID, TITLE, ...) VALUES (43, 1, 'Fix login bug', ...)
```

### Why IDENTITY and not SEQUENCE for pmt?

**1. Write volume is human-paced, not machine-paced.**

pmt is a collaboration tool. Users create tickets, send messages, and add members one action at a time — there are no bulk-insert batch jobs, no event-sourcing pipelines, no data imports. The primary performance advantage of `SEQUENCE` (enabling JDBC batch INSERTs) simply never activates in this workload.

**2. Zero annotation boilerplate — one base class for all 8 tables.**

With `SEQUENCE`, every entity needs its own `@SequenceGenerator` binding:

```java
// Must be repeated for Ticket, User, Project, ChatMessage, ...
@GeneratedValue(strategy = GenerationType.SEQUENCE, generator = "ticket_seq")
@SequenceGenerator(name = "ticket_seq", sequenceName = "ticket_id_seq", allocationSize = 50)
private Integer id;
```

With `IDENTITY`, the declaration lives once in `BaseEntity` — all 8 entities inherit it with zero extra annotations.

**3. `IDENTITY` is the exact behavioral match for `GENERATED BY DEFAULT AS IDENTITY`.**

Hibernate's `GenerationType.IDENTITY` tells it: *"do not generate or pre-fetch the ID — let the DB column handle it after INSERT"*. This directly corresponds to how PostgreSQL identity columns work. Using `SEQUENCE` here would mean Hibernate calls `nextval()` manually, bypassing the column's built-in identity sequence — adding unnecessary complexity.

**4. `Integer` (4 bytes) is correctly sized for the domain.**

`Integer` maps to PostgreSQL `INT` (up to ~2.1 billion rows). For a project management tool, this ceiling will never be hit. Using `Long` (8 bytes) would double the storage cost for every PK and FK column across all 8 tables for no practical gain.

**5. ID gaps from `SEQUENCE` pre-allocation are undesirable.**

`SEQUENCE` with `allocationSize=50` discards the pre-fetched block on every application restart. In development and CI/CD with frequent restarts, IDs would jump in gaps of 50 — making the database harder to inspect and debug. With `IDENTITY`, IDs are always strictly sequential per table.
