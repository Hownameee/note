...
Sysstat operates by reading kernel performance counters exposed via the `/proc` virtual filesystem and saving them to disk using a lightweight binary format.

```mermaid
graph TD
    K[Linux Kernel /proc] -->|Reads counters| SADC[sadc - Data Collector]
...
1.  **Metric Exposure (`/proc`):** The Linux kernel maintains memory counters for CPU activity in `/proc/stat`, virtual memory in `/proc/vmstat` and `/proc/meminfo`, disk activity in `/proc/diskstats`, and network statistics in `/proc/net/dev`.
